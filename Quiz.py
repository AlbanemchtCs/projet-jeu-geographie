...
class Quiz():
    def __init__(self, init_country=None, corpus={}):
        if(init_country is None):
            ...
            self.current_country_id = DATAFRAME_COUNTRIES[DATAFRAME_COUNTRIES['country_name']
                                                          == init_country].index[0]
        self.points = 0
        self.corpus = corpus
        self.validated_countries = []
        self.lost = False
        self.potential_question = set(range(len(self.corpus)))
        self.validate_current_country()
    """
    ...
    def next_country(self):
        ...
        if(len(neighbors) > 0):
            neighbors_kept = neighbors[~neighbors['country_name'].isin(
                self.validated_countries)]
        else:
            neighbors_kept = neighbors
        # If there isn't other neighbor the quiz select a new random country
        if(len(neighbors_kept) == 0):
            sample = DATAFRAME_COUNTRIES[~DATAFRAME_COUNTRIES['country_name'].isin(
            ...
            while(retry):
                ...
                if(len(match) > 0):
                    ...
                    # Case where answer is accepted
                        ...
                        print(
                            'Bravo! ' + match[0] + ' est bien un voisin de ' + self.current_country_name)
                        self.current_country_id = neighbors.index[neighbors_list.index(
                            match[0])]
    ...
    def next_question(self):
        ...
        while(finding_question and len(self.potential_question) > 0):
            ...
            results = result_query(
                self.corpus[sample]['query'], self.current_country_id)
        ...
        # Case where a question is found
            ...
            answer = input(self.corpus[sample]['question'].format(
                self.current_country_name) + ' : ')
            # Numerical case
            if(self.corpus[sample]['answer_type'] in [int, float]):
                ...
                if(convert and abs(float(answer)-float(results[0])) <= float(results[0])*self.corpus[sample]['error_ratio']):
                    ...
                    print(
                        "Bonne réponse ! La réponse exacte était " + results[0])
                ...
                # Answer is wrong
                    ...
                    self.lost = True
                    print(
                        "Mauvaise réponse... La réponse exacte était " + results[0])
                    return False
            # String case
            elif(self.corpus[sample]['answer_type'] is str):
                match = get_close_matches(
                    answer, results, cutoff=THRESHOLD_SIMILARITY)
                # Answer is valid
                if(len(match) >= 1):
                    self.points += POINT_QUESTION
